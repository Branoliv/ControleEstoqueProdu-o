        ...
        private void btnPgtoCompraLocalizar_Click(object sender, EventArgs e)
        {
            ...
            if (f.codigo != 0)
            {
                ...
                try
                {
                    ...
                    txbPgtoComCod.Text = modelo.ComCod.ToString("D6");
                ...
                try
                    ...
                    for (int i = 0; i < tabelap.Rows.Count; i++)
                    {
                        ...
                        DateTime? pdtPagto =null;
                        
                        if (tabelap.Rows[i]["pco_datapagto"] != DBNull.Value)
                        {
                            //pdtPagto = null ;                            
                           // pdtPagto.Value.Date.ToShortDateString() = (DateTime)Convert.ToDateTime(tabelap.Rows[i]["pco_datapagto"]);
                        }
                        else
                        {
                        ...
                        String[] it = new String[] { status.ToString(), pcod.ToString("D2"), pvlpar.ToString("N2"), pdtPagto.ToString(), pdtvenc.Date.ToString("d") };
