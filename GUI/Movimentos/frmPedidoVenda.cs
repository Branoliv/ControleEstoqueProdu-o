    ...
    public partial class frmPedidoVenda : Form
    {
        ...
        public void alterabotoes(int op)
        {
            ...
            if (op == 5)
            {
                btnCancelar.Enabled = true;
                gbxPedVendDadosCli.Enabled = true;
                dtpkPedVenData.Enabled = false;
                txbPedVenCodCli.Enabled = false;
                lblPedVenCliNome.Enabled = false;
                dtpkPedVenDtEntrega.Enabled = false;
        ...
        private void btnLocalizar_Click(object sender, EventArgs e)
        {
            ...
            if(f.codigo != 0)
            {
                ...
                try
                {
                    ...
                    {
                        btnEstorno.Enabled = false;
                        lblPedVendAviso.Text = "Pedido Cancelado";
                        lblPedVendAviso.ForeColor = Color.Red;
                        lblPedVendAviso.Visible = true;
                        alterabotoes(5);
                    }
                    else if(modelo.PedStatus == 3)
                    {
                        btnEstorno.Enabled = false;
                        lblPedVendAviso.Text = "Ordem Criada";
                        lblPedVendAviso.ForeColor = Color.Green;
                        lblPedVendAviso.Visible = true;
                        alterabotoes(5);
                    }
                    else
        ...
        private void btnEstorno_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                modelo.PedCod = Convert.ToInt32(txbPedVenCod.Text);
                modelo.PedStatus = 1;
                bll.AtualizaStatus(modelo);

                for (int i = 0; i < dgvPedVenItens.RowCount; i++)
                {
                    modeloIpv.IpvCod = Convert.ToInt32(dgvPedVenItens.Rows[i].Cells[0].Value);
                    modeloIpv.PedCod = modelo.PedCod;
                    modeloIpv.IpvStatus = 1;
                    bllIpv.CancelarPedido(modeloIpv);
                }

                LimpaTela();
                alterabotoes(1);
                MessageBox.Show("Pedido cancelado com sucesso." , "Aviso", MessageBoxButtons.OK, MessageBoxIcon.None);
                cx.TerminarTransacao();
            }
            catch (Exception ex)
            {
                MessageBox.Show("Não foi possível cancelar o pedido." + ex.Message, "Erro", MessageBoxButtons.OK, MessageBoxIcon.Error);
                cx.CancelarTransacao();
            }
            finally
            {
        ...
        public void CarregaProduto()
        {
            try
            {
                ...
                {
                    MessageBox.Show("Produto inativo. ", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    lblPedVenProdNome.Text = "Informe o código do produto.";
                    txbPedVenCodProd.Focus();
                }
                else
                {
                    ...
                    txbPedVenVlUnit.Text = modelo.ProdValorpago.ToString("N2");
                }
            }
            catch (Exception ex)
            {
        ...
        private void btnPedVenProdIncluir_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                for (int x = 0; x < dgvPedVenItens.Rows.Count; x++)
                {
                   j  = Convert.ToInt32(dgvPedVenItens.Rows[x].Cells[0].Value);
        ...
        private void dgvPedVenItens_CellValidated(object sender, DataGridViewCellEventArgs e)
        {
            Double totalLinha = Convert.ToDouble(dgvPedVenItens.Rows[e.RowIndex].Cells[3].Value) * Convert.ToDouble(dgvPedVenItens.Rows[e.RowIndex].Cells[6].Value);
            dgvPedVenItens.Rows[e.RowIndex].Cells[7].Value = totalLinha.ToString("N2");
            CalculaValorPeido();

            double d = double.Parse(dgvPedVenItens.Rows[e.RowIndex].Cells[3].Value.ToString());
            dgvPedVenItens.Rows[e.RowIndex].Cells[3].Value = d.ToString("N3");
        }
    }
}
